...
Map<String, String> _vault({bool pin = false}) => {
      ..._onboarded,
      if (pin) StorageKeys.pinHash: 'x',
      StorageKeys.password('Gmail'):
          _password('Gmail', 'Sup3r-Secret-Passw0rd!', 'Personal', 0xFFFFEB3B),
      StorageKeys.password('Company VPN with a very long application name'):
          _password('Company VPN with a very long application name', 'x',
              'Work', 0xFF64B5F6),
      StorageKeys.password('Twitter'):
          _password('Twitter', 'abc', 'Social', 0xFFE57373),
      StorageKeys.password('Bank'):
          _password('Bank', '1234', 'Finance', 0xFF81C784),
      StorageKeys.password('Netflix'):
          _password('Netflix', 'n', 'General', 0xFFBA68C8),
      StorageKeys.password('Steam'):
          _password('Steam', 's', 'General', 0xFFFFB74D),
...
void _flow(
  ...
  /// Registers test doubles in a GetIt scope that lives for this flow only.
  void Function()? overrides,
}) {
  for (final entry in _sizes.entries) {
    testWidgets('$name @ ${entry.key}', (tester) async {
      ...
      try {
        ...
        tester.view.reset();
        if (overrides != null) await getIt.popScope();
      }
...
void main() {
  ...
  // --- Google Drive backup (fake Drive; deterministic recovery key) -------

  late FakeBackupRepository backup;
  final recoveryKey = RecoveryKey.generate(Random(42));

  void backupOverrides({required bool enabled}) {
    backup = FakeBackupRepository(
      enabled: enabled,
      // Relative time only: "5 min ago" on every run.
      lastSuccessAt: DateTime.now().subtract(const Duration(minutes: 5)),
    )..backupKey = recoveryKey;
    getIt
      ..registerLazySingleton<IBackupRepository>(() => backup)
      ..registerFactory<BackupSettingsBloc>(
        () => BackupSettingsBloc(
          backup,
          getIt<ILockRepository>(),
          random: Random(7),
        ),
      );
  }
  ...
  _flow(
    'backup setup',
    _vault(),
    overrides: () => backupOverrides(enabled: false),
    (s) async {
      await s.tap(find.byIcon(Icons.person_outline_rounded));
      await s.tap(find.byIcon(Icons.cloud_outlined));
      await s.golden('20_backup_intro');
      await s.tap(find.text('Set up backup'));
      await s.golden('21_backup_words');
      await s.tester.scrollUntilVisible(
        find.text("I've written them down"),
        200,
        // The page list, not a TextField's own Scrollable.
        scrollable: find
            .descendant(
      ...
      await s.tap(find.text("I've written them down"));
      await s.golden('22_backup_confirm');
    },
  );
  ...
  _flow(
    'backup status',
    _vault(),
    overrides: () => backupOverrides(enabled: true),
    (s) async {
      await s.tap(find.byIcon(Icons.person_outline_rounded));
  ...
  _flow(
    ...
    (s) async {
      for (var page = 0; page < 2; page++) {
        await s.tap(find.byIcon(Icons.skip_next_outlined));
      }
      await s.golden('24_onboarding_last_page');
      await s.tap(find.text('Restore from Google Drive'));
      await s.golden('25_restore_connect');
      await s.tap(find.text('Connect Google account'));
      // Dates depend on time zone and clock: text checks only.
      expect(find.text('Choose a backup'), findsOneWidget);
      expect(find.textContaining('Latest'), findsOneWidget);
      await s.tap(find.text('Continue'));
      await s.golden('26_restore_key');
      await s.tester.enterText(
        find.byType(TextField).first,
        recoveryKey.words.join(' '),
      );
      await s.settle(5);
      await s.tester.scrollUntilVisible(
        find.text('Open backup'),
        200,
        // The page list, not a TextField's own Scrollable.
        scrollable: find
            .descendant(
      ...
      await s.tap(find.text('Open backup'));
      expect(find.text('Backup contents'), findsOneWidget);
      expect(find.text('0 passwords · 0 authenticator codes'), findsOneWidget);
    },
  );
}
