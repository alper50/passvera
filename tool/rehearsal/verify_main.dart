...
Future<void> main() async {
  ...
  // Same startup step as the app (see InitializeApp.prepareStorage).
  (await getIt<IStorageMigration>().migrate()).fold((f) => throw f, (_) {});
  ...
  if ((await getIt<KeysService>().isOnboardCompleted())
          .getOrElse(() => false) !=
      true) {
    problems.add('onboard flag missing');
  }

  // Storage layout: migration finished and every key is namespaced.
  final raw = await getIt<KeysService>().storage.readAll();
  if (raw[StorageKeys.schema] != StorageKeys.currentSchema) {
    problems.add('schema marker ${raw[StorageKeys.schema]}');
  }
  final bare = raw.keys.where(
    (k) =>
        !k.startsWith(StorageKeys.passwordPrefix) &&
        !k.startsWith(StorageKeys.metaPrefix) &&
        !k.startsWith(StorageKeys.totpPrefix),
  );
  if (bare.isNotEmpty) problems.add('un-namespaced keys $bare');
  if (raw.containsKey(StorageKeys.migrationJournal)) {
    problems.add('migration journal left behind');
  }
