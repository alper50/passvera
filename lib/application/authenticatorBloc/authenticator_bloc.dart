  ...
  AuthenticatorBloc(this._repository) : super(AuthenticatorState.initial()) {
    on<AuthenticatorEvent>((event, emit) async {
      await event.map(
        ...
        addFromUri: (e) async {
          emit(state.copyWith(isLoading: true, addFailureOrSuccess: none()));
          final result = await _repository.addFromOtpAuthUri(e.uri);
          await result.fold(
            (failure) async => emit(
              state.copyWith(
                isLoading: false,
        ...
        delete: (e) async {
          emit(state.copyWith(isLoading: true, deleteFailureOrSuccess: none()));
          final result = await _repository.delete(id: e.id);
          await result.fold(
            (failure) async => emit(
              state.copyWith(
                isLoading: false,
